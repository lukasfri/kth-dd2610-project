...
class Transformer(nn.Module):
  ...
  vocab_size: int
  num_classes: int  # <--- NEW: Total classes (e.g. 10)
  hidden_size: int = 768
  ...
  def __call__(self,
               ...
               class_labels: jnp.ndarray, # <--- NEW: Input argument
    ...
    # We add +1 to num_classes for the "Unconditional" (dropped) label used in CFG

    class_embedder = nn.Embed(
        num_embeddings=self.num_classes + 1, 
        features=self.hidden_size,
        embedding_init=truncated_normal(self.initializer_range),
        name='class_embeddings')

    # Expand class embeddings to shape (Batch, 1, Hidden) and add to image tokens
    class_emb = class_embedder(class_labels)[:, None, :]

    layer_input = input_embeddings + class_emb
